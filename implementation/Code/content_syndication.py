...
class DataCleaner:
    def __init__(self, input_csv, picklist_xlsx, output_dir,fid,mysql):
    ...
    def filter_import_type(self):
        self.removed_rows["import_type"] = self.df[self.df["Import Type"] != "nonpartner"]
        self.df = self.df[self.df["Import Type"] == "nonpartner"]
        print("Executed")
    ...
    def validate_picklist_values(self):
        ...
        valid_sfdc_campaign_type = picklist_df["Import SFDC Campaign Type"].dropna().unique()
        valid_utm_import_source = picklist_df["UTM Import Source"].dropna().unique()

        # Define validation rules for "Import Record Type"
        rules = { 
            "WF": {"qa_required": False, "comments_required": False},
            "1-Touch": {"qa_required": False, "comments_required": False},
            "2-Touch": {"qa_required": False, "comments_required": False},
            "BANT": {"qa_required": True, "comments_required": False},
            "SRL": {"qa_required": True, "comments_required": False},
            "HQL": {"qa_required": True, "comments_required": False},
            "SQL": {"qa_required": True, "comments_required": False},
            "1-Touch-Profiler": {"qa_required": False, "comments_required": True},
            "2-Touch-Profiler": {"qa_required": False, "comments_required": True},
        }

        def check_import_record_type(row):
            record_type = row["Import Record Type"]
            if record_type not in rules:
                return True  # Invalid record type
    
            rule = rules[record_type]
    
            # Ensure "WF", "1-Touch", and "2-Touch" do not have QA, Comments, or Subjects
            if record_type in ["WF", "1-Touch", "2-Touch"]:
                if not pd.isna(row["Question 01"]) or not pd.isna(row["Answer 01"]) or not pd.isna(row["Comments"]) or not pd.isna(row["Subjects"]):
                    return True  # Invalid if any of these fields are present
    
            # Ensure QA fields are present when required
            if rule["qa_required"] and (pd.isna(row["Question 01"]) or pd.isna(row["Answer 01"])):
                return True  # QA is required but missing
    
            # Ensure Comments are present when required
            if rule["comments_required"] and pd.isna(row["Comments"]):
                return True  # Comments required but missing
        ...
        invalid_rows = (
            ...
            ~self.df["Import SFDC Campaign Type"].isin(valid_sfdc_campaign_type) |
            ~self.df["UTM Import Source"].isin(valid_utm_import_source) |
            self.df.apply(check_import_record_type, axis=1)  # Validate Import Record Type rules
        )
    ...
    def remove_blank_rows(self):
        required_cols = ["Import SFDC Campaign Type", "Import SFDC Campaign Id", "Import Type", "First Name", "Last Name", "Email Address", "Job Title", "Country", "State or Province", "Zip or Postal Code", "Phone"]
    ...
    def remove_exclude_routing(self):
        ...
        self.removed_rows["exclude_routing"] = self.df[self.df["Exclude Routing"] == "True"]
        self.df = self.df[self.df["Exclude Routing"] != "True"]
    ...
    def save_to_db(self, file_path, table_name,error_type=None):
        ...
        try:
            if table_name == "converted_file":
                ...
                values=(self.fid, f"converted_file_{self.fid}", file_path, "content-syndication")
                cursor.execute(query, values)

            elif table_name == "error":
                query = "INSERT INTO error (fid, ename, epath, type, generated_date) VALUES (%s, %s, %s, %s, NOW())"
                values=(self.fid, f"error_{error_type}_{self.fid}", file_path, "content-syndication")
                cursor.execute(query, values)
