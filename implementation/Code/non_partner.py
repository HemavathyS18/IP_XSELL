...
class DataCleaner:
    ...
    def validate_sales_followup(self):
        # Step 2: Limit "Sales Follow-Up" rows to 50
        self.column_name = "Import SFDC Campaign Status"

        # Filter rows where the column has "Sales Follow-Up"
        self.sales_follow_up_rows = self.df[self.df[self.column_name] == "Sales Follow-Up"]

        # Split into allowed (first 50) and exceeded rows
        self.allowed_rows = self.sales_follow_up_rows.head(50)
        self.exceeded_rows = self.sales_follow_up_rows.iloc[50:]

        # Keep only valid rows in df (remove all "Sales Follow-Up" rows first)
        self.df = self.df[self.df[self.column_name] != "Sales Follow-Up"]

# Add back the allowed 50 "Sales Follow-Up" rows
        self.df = pd.concat([self.df, self.allowed_rows], ignore_index=True)
        self.removed_rows["exceeded_sales_follow_up"] = self.df[self.exceeded_rows]
