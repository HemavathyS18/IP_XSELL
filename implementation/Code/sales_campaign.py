...
class DataCleaner:
    ...
    def filter_import_type(self):
        self.removed_rows["import_type"] = self.df[self.df["Import Type"] != "sales"]
        self.df = self.df[self.df["Import Type"] == "sales"]
        print("Executed")
    ...
    def validate_picklist_values(self):
        ...
        valid_campaign_types = picklist_df["Import SFDC Campaign Type"].dropna().unique()
        ...
        invalid_rows = (
            ...
            ~self.df["Import SFDC Campaign Status"].isin(valid_sfdc_status) | ~self.df["Import SFDC Campaign Type"].isin(valid_campaign_types) 
        )
    ...
    def validate_opt_in_flag(self):
        self.removed_rows["opt_in_flag_zero"] = self.df[self.df["Opt-In Flag"] .notna()]
        self.df = self.df[self.df["Opt-In Flag"] .isna()]
    ...
    def remove_blank_rows(self):
        required_cols = ["Import SFDC Campaign Type", "Import SFDC Campaign Id", "Import SFDC Campaign Status", "Import Type", "First Name", "Last Name", "Email Address", "Job Title", "Country", "State or Province", "Zip or Postal Code", "Phone"]
    ...
    def remove_exclude_routing(self):
        ...
        self.df["Exclude Routing"] = self.df["Exclude Routing"].replace({1: "TRUE"})
